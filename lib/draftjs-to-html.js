...
*/
function blockTypesMapping(tagsMapping) {
  return {
...
*/
function getBlockTag(type, tagsMapping) {
...
*/
function getBlockMarkup(block, entityMap, hashtagConfig, directional, customEntityTransform, tagsMapping) {
  var blockHtml = [];
  if (isAtomicEntityBlock(block)) {
    blockHtml.push(getEntityMarkup(entityMap, block.entityRanges[0].key, undefined, customEntityTransform));
  } else {
    var blockTag = getBlockTag(block.type, tagsMapping);
    if (blockTag) {
...
*/
function getListMarkup(listBlocks, entityMap, hashtagConfig, directional, customEntityTransform, tagsMapping) {
  var listHtml = [];
  var nestedListBlock = [];
  ...
  listBlocks.forEach(function (block) {
    ...
    if (!previousBlock) {
      ...
      if (nestedListBlock && nestedListBlock.length > 0) {
        listHtml.push(getListMarkup(nestedListBlock, entityMap, hashtagConfig, directional, customEntityTransform, tagsMapping));
        nestedListBlock = [];
      }
    } else {
      nestedBlock = true;
...
*/
function draftToHtml(editorContent, hashtagConfig, directional, customEntityTransform, tagsMapping) {
  var html = [];
      ...
      blocks.forEach(function (block) {
        if (isList(block.type)) {
          ...
          if (listBlocks.length > 0) {
            var listHtml = getListMarkup(listBlocks, entityMap, hashtagConfig, customEntityTransform, tagsMapping); // eslint-disable-line max-len
            html.push(listHtml);
            listBlocks = [];
          }
          var blockHtml = getBlockMarkup(block, entityMap, hashtagConfig, directional, customEntityTransform, tagsMapping);
          html.push(blockHtml);
        }
      });
      if (listBlocks.length > 0) {
        var listHtml = getListMarkup(listBlocks, entityMap, hashtagConfig, directional, customEntityTransform, tagsMapping); // eslint-disable-line max-len
        html.push(listHtml);
        listBlocks = [];
      }
    }
